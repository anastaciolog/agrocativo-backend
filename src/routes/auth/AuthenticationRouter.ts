...
import { sendResetPasswordEmail, sendWelcomeEmail } from "../../services/Mailer"
...
export class AuthenticationRouter implements RouterInterface, AuthenticationRouterInterace {
	...
	public async login(request: Request, response: Response, next: NextFunction): Promise<void> {
		...
		if (!this.validator.validateEmail(email)) {
			next(new ErrorResponse(3188, "Invalid email", 400))
			return
		}

		if (!this.validator.validatePlatform(platform)) {
			next(new ErrorResponse(3188, "Invalid platform", 400))
			return
		}

		if (code && code !== "" && !this.validator.validateCode(code)) {
			next(new ErrorResponse(3189, "Invalid confirmation code", 400))
			return
		}
		...
		try {
			...
			if (!user) {
				next(new ErrorResponse(3112, "User not found", 401))
				return
			}

			if (!user.confirmed && code === "") {
				next(new ErrorResponse(3119, "Account not activated", 401))
				return
			}
			...
			if (!match) {
				next(new ErrorResponse(3113, "Incorrect password", 401))
				return
			}

			//Activated account if not activate
			if (!user.confirmed && (code !== user.confirmation_code)) {
				next(new ErrorResponse(3134, "Invalid confirmation code", 400))
				return
			}
	...
	public async register(request: Request, response: Response, next: NextFunction): Promise<void> {
		...
		if (!this.validator.validateCPF(cpf)) {
			next(new ErrorResponse(3117, "Invalid cpf", 400))
			return
		}

		if (!this.validator.validateName(firstname)) {
			next(new ErrorResponse(3116, "Invalid firstname", 400))
			return
		}


		if (!this.validator.validateName(lastname)) {
			next(new ErrorResponse(3115, "Invalid lastname", 400))
			return
		}

		if (!this.validator.validateEmail(email)) {
			next(new ErrorResponse(3118, "Invalid email", 400))
			return
		}

		if (!this.validator.validateCelular(celular)) {
			next(new ErrorResponse(3114, "Invalid celular", 400))
			return
		}

		if (!this.validator.validateDataNasc(birthday)) {
			next(new ErrorResponse(3119, "Invalid birthday", 400))
			return
		}

		if (!this.validator.validatePassword(password)) {
			next(new ErrorResponse(3120, "Invalid password", 400))
			return
		}

		if (!this.validator.validateGender(gender)) {
			next(new ErrorResponse(3121, "Invalid gender", 400))
			return
		}
		...
		try {
			...
			if (verifyEmail) {
				next(new ErrorResponse(3122, "The email informed is already in use by another account!", 500))
				return
			}
			...
			const sendmail = await sendWelcomeEmail(user, confirmation_code)

			if (sendmail instanceof Error) {
				next(new ErrorResponse(3131, "Error in send mail process", 500))
				return
			}

			response.json({
				message: "Congrulations! You account was created successfuly!"
			})
	...
	public async confirmedLogin(request: Request, response: Response, next: NextFunction): Promise<void> {
		...
		try {
			...
			if (!user) {
				next(new ErrorResponse(3135, "User not found", 400))
				return
			}
			...
			// Success
			response.json({
				message: "Congrulations! You account was validated successfuly!"
			})
	...
	public async updateSession(request: Request, response: Response, next: NextFunction): Promise<void> {
		...
		try {
			...
			if (!updatedSession) {
				next(new ErrorResponse(3126, "Invalid token", 400))
				return
			}
	...
	public async resetPassword(request: Request, response: Response, next: NextFunction): Promise<void> {
		...
		try {
			...
			if (!user) {
				next(new ErrorResponse(3129, "User not found", 400))
				return
			}
			...
			if (!updatedUser.confirmation_code) {
				next(new ErrorResponse(3131, "Error generating confirmationCode", 500))
				return
			}

			const sendmail = sendResetPasswordEmail(user, updatedUser.confirmation_code)
			...
			// Success
			response.json({
				message: "We send you an email with instructions to reset your password!"
			})
